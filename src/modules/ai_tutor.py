...
LLM 调用策略：
  ...
  配置来源优先级：环境变量 > st.secrets（Streamlit Cloud）。
  支持的配置项：
...
def get_llm_config() -> dict | None:
    """读取 LLM 配置，优先环境变量，其次 st.secrets。

    返回 None 表示未配置 API Key，调用方应走模板路径。
    """
    api_key = os.environ.get("OPENAI_API_KEY")
    base_url = os.environ.get("OPENAI_BASE_URL")
    model = os.environ.get("RECKONBOX_LLM_MODEL", "gpt-4o-mini")
    ...
    try:
        ...
        if hasattr(st, "secrets"):
            ...
            if not base_url and "openai_base_url" in st.secrets:
